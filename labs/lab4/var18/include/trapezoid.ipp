...
namespace figure {
    template<Scalar T>
    bool Trapezoid<T>::Validate(const Point<T>& p1, const Point<T>& p2, const Point<T>& p3, const Point<T>& p4) const {
    ...
    template<Scalar T>
    std::unique_ptr<T[]> Trapezoid<T>::GetLengths() const {
    ...
    template<Scalar T>
    Trapezoid<T>::Trapezoid(): Trapezoid(Point<T>(0.0, 0.0), Point<T>(0.0, 1.0), Point<T>(1.0, 1.0), Point<T>(1.0, 0.0)) {}

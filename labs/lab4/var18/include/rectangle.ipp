...
namespace figure {
    template<Scalar T>
    bool Rectangle<T>::Validate(const Point<T>& p1, const Point<T>& p2, const Point<T>& p3, const Point<T>& p4) const {
    ...
    template<Scalar T>
    Rectangle<T>::Rectangle(): Rectangle(Point<T>(0.0, 0.0), Point<T>(0.0, 1.0), Point<T>(1.0, 1.0), Point<T>(1.0, 0.0)) {}
